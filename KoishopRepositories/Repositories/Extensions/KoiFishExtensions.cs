...
namespace KoishopRepositories.Repositories.Extensions
{
  public static class KoiFishExtensions
  {
    ...
    public static IQueryable<KoiFish> Filter(this IQueryable<KoiFish> query, KoiFishParams koiFishParams)
    {
      ...
      if (koiFishParams.BreedName != null && koiFishParams.BreedName.Any())
      {
        var lowerCaseBreedName = koiFishParams.BreedName.Select(g => g.ToLower()).ToList();
        query = query.Where(k => lowerCaseBreedName.Contains(k.Breed.BreedName.ToLower()));
      }

      if (koiFishParams.UserId != 0)
        query = query.Where(k => k.UserId == koiFishParams.UserId);
      return query;
    }
  }
}

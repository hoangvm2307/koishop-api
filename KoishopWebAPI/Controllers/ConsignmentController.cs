...
public class ConsignmentController : BaseApiController
{
  ...
  /// <returns></returns>
  [HttpGet("user/{id}")]
  public async Task<ActionResult<ConsignmentDto>> GetConsignmentByUserId(int id)
  {
    var consignment = await _consignmentService.GetListConsignmentByUserId(id);
    if (consignment == null)
      return NotFound();
    return Ok(consignment);
  }
  ...
  [HttpPut("cancel/{id}")]
  public async Task<ActionResult> CancelConsignment(int id)
  {
    var isCancelled = await _consignmentService.CancelConsignment(id);
    if (!isCancelled)
      return NotFound();
    return NoContent();
  }
